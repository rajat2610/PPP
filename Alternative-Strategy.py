...
for index, row in signal_temp.iterrows():
    signal.loc[index, row[row>under].index] = 1
for index, row in signal_temp.iterrows():
    signal.loc[index, row[row<over].index] = -1
        
asset_returns = signal * returns

count = 9 - (signal == 0).astype(int).sum(axis = 1)
portfolio_returns = (asset_returns.sum(axis = 1))/count
